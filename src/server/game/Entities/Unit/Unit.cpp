...
{
    WorldPacket data(SMSG_MONSTER_MOVE, 1+12+4+1+4+4+4+12+GetPackGUID().size());  //Checked for 406a
...
void Unit::GetRandomContactPoint(const Unit* obj, float &x, float &y, float &z, float distance2dMin, float distance2dMax) const
{
    ...
    if (attacker_number > 0)
        --attacker_number;
    GetNearPoint(obj, x, y, z, obj->GetCombatReach(), distance2dMin+(distance2dMax-distance2dMin) * (float)rand_norm(), GetAngle(obj) + (attacker_number ? (static_cast<float>(M_PI/2) - static_cast<float>(M_PI) * (float)rand_norm()) * float(attacker_number) / combat_reach * 0.3f : 0));
}
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            ...
            if (crit)
            {
                ...
                if (Player* modOwner = GetSpellModOwner())
                    modOwner->ApplySpellMod(spellInfo->Id, SPELLMOD_CRIT_DAMAGE_BONUS, crit_bonus, NULL, victim);
                damage += crit_bonus;
...
void Unit::DealMeleeDamage(CalcDamageInfo *damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->damage)
    {
        ...
        for (AuraEffectList::const_iterator dmgShieldItr = vDamageShieldsCopy.begin(); dmgShieldItr != vDamageShieldsCopy.end(); ++dmgShieldItr)
        {
            ...
            WorldPacket data(SMSG_SPELLDAMAGESHIELD, (8+8+4+4+4+4)); // 406a
...
{
    WorldPacket data(SMSG_EMOTE, 4 + 8);  //406a
    data << uint32(anim_id);
    data << uint64(GetGUID());
    SendMessageToSet(&data, true);
}
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    if (spellInfo)
        if (Player* modOwner = GetSpellModOwner())
            modOwner->ApplySpellMod(spellInfo->Id, SPELLMOD_IGNORE_ARMOR, armor, NULL, victim);
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (AuraEffectList::const_iterator itr = ArPenAuras.begin(); itr != ArPenAuras.end(); ++itr)
        {
            // item neutral spell
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo)
{
    ...
    if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0)
    {
        ...
        static uint32 const BOSS_LEVEL = 88;
...
{
    WorldPacket data(SMSG_ATTACKSTART, 8 + 8);  //406
    data << uint64(GetGUID());
    data << uint64(victim->GetGUID());
    SendMessageToSet(&data, true);
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (Player* modOwner = GetSpellModOwner())
        modOwner->ApplySpellMod(spell->Id, SPELLMOD_RESIST_MISS_CHANCE, modHitChance, NULL, victim);
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            else
            {
                ...
                if (Aura* newAura = Aura::TryRefreshStackOrCreate(aura->GetSpellInfo(), effMask, stealer, NULL, &baseDamage[0], NULL, aura->GetCasterGUID()))
                {
                    ...
                    // FIXME: using aura->GetMaxDuration() maybe not blizz-like but it fixes stealing of spells like Innervate
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    uint32 chance = 20;
                    player->ApplySpellMod(dummySpell->Id, SPELLMOD_CHANCE_OF_SUCCESS, chance, NULL, victim);
...
void Unit::SetMinion(Minion *minion, bool apply, PetSlot slot)
{
    ...
    if (apply)
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
        {
            // If its a Hunter Pet, set pet slot and setPetSlotUsed.
...
uint32 Unit::SpellDamageBonus(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (DoneAdvertisedBenefit || TakenAdvertisedBenefit)
    {
        ...
        {
            coeff *= 100.0f;
            modOwner->ApplySpellMod(spellProto->Id, SPELLMOD_BONUS_MULTIPLIER, coeff, NULL, victim);
            coeff /= 100.0f;
        }
    ...
    if (Player* modOwner = GetSpellModOwner())
        modOwner->ApplySpellMod(spellProto->Id, damagetype == DOT ? SPELLMOD_DOT : SPELLMOD_DAMAGE, tmpDamage, NULL, victim);
...
uint32 Unit::SpellHealingBonus(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (Player* modOwner = GetSpellModOwner())
        modOwner->ApplySpellMod(spellProto->Id, damagetype == DOT ? SPELLMOD_DOT : SPELLMOD_DAMAGE, heal, NULL, victim);
...
void Unit::MeleeDamageBonus(Unit* victim, uint32 *pdamage, WeaponAttackType attType, SpellInfo const* spellProto)
{
    ...
    if (spellProto)
        if (Player* modOwner = GetSpellModOwner())
            modOwner->ApplySpellMod(spellProto->Id, SPELLMOD_DAMAGE, tmpDamage, NULL, victim);
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    // can't attack non-attackable units or GMs
    ...
    // can't attack is not targetable
    ...
    if (GetReactionTo(target) == REP_NEUTRAL && target->GetReactionTo(this) == REP_NEUTRAL)
    {
        if (!(target->GetTypeId() == TYPEID_PLAYER && GetTypeId() == TYPEID_PLAYER) && !(target->GetTypeId() == TYPEID_UNIT && GetTypeId() == TYPEID_UNIT))
        {
...
void Unit::setDeathState(DeathState s)
{
    ...
    if (s == JUST_DIED)
    {
        ...
        SetPower(getPowerType(), 0);
         ...
         RemoveFlag (UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE); // clear skinning for creature and player (at battleground)
...
void Unit::SetMaxPower(Powers power, int32 val)
{
    ...
    SetInt32Value(UNIT_FIELD_MAXPOWER1 + power, val);
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    //        pvp->HandlePlayerActivityChangedvictim->ToPlayer();
...
void SpellModifier::Recalculate(SpellInfo const *spellInfo, Unit* target)
{
    if (ownerAuraEffect)
